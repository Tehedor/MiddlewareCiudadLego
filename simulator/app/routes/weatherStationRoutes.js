...
const { changeWeatherStation } = require('../utils/controlJSON.js');
...
simulationRoutes.post('/weatherStation/toggle', function (req, res) {
    ...
    try {
        const stateChanged = changeWeatherStation(state);
        if (stateChanged) {
            if (state) {
