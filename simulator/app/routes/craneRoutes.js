...
const { changeUltrasoundSensor } = require('../utils/controlJSON.js');
...
simulationRoutes.post('/crane/toggle', function (req, res) {
    ...
    try {
        const stateChanged = changeUltrasoundSensor(state);
        if (stateChanged) {
            if (state) {
