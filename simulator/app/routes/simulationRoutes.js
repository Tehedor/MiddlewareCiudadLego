...
const { changeInicialState } = require('../utils/controlJSON.js');
...
simulationRoutes.post('/simulation/toggle', function (req, res) {
    ...
    try {
        const stateChanged = changeInicialState(state);
        if (stateChanged) {
            if (state) {
